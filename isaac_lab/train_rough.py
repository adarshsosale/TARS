"""Project-level training wrapper for the Hexabot ROUGH-TERRAIN teacher (Milestone 1).

Same as train_hexabot.py (registers the task from isaac_lab/tasks/, routes logs to
<project_root>/logs/), but:
  * defaults to the rough task `Isaac-Velocity-Rough-Hexabot-Direct-v0`;
  * injects the distillable terrain-teacher policy class
    (`HexabotTeacherActorCritic`) into rsl_rl's runner namespace so the agent cfg's
    `class_name="HexabotTeacherActorCritic"` resolves (rsl_rl resolves the policy
    class by `eval(class_name)` in its on_policy_runner module globals).
...
# Route logs/video output to <project_root>/logs/ by making the project root the cwd.
...
parser = argparse.ArgumentParser(description="Train the Hexabot rough-terrain teacher with RSL-RL.")
...
# Inject the distillable terrain-teacher into rsl_rl's runner namespace so
# `eval("HexabotTeacherActorCritic")` in OnPolicyRunner resolves to our class.
...
import hexabot  # noqa: F401  — registers the Hexabot tasks
