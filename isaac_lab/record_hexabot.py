"""Record a video of the Hexabot hexapod walking with the TRAINED PPO policy.

Loads the TorchScript policy exported by play_hexabot.py (logs/.../exported/policy.pt),
runs a single robot on flat ground with grippy contact friction (matching
training), commands a fixed forward velocity, and records a tracking-camera mp4.

Usage (via isaaclab.sh -p):
    record_hexabot.py --policy <path/to/exported/policy.pt> [--seconds 10] [--cmd_vx 0.2]
"""
...
parser.add_argument("--out", type=str, default="/home/adarshsosale/Workspace/Isaac RL Lab/hexabot_model/hexabot_locomotion.mp4")
