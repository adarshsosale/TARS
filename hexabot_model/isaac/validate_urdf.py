#!/usr/bin/env python3
"""
validate_urdf.py — offline sanity checks for hexabot.urdf, before it ever touches
Isaac Sim.  Pure stdlib + numpy (no ROS / Isaac needed).

  python3 hexabot_model/isaac/validate_urdf.py

Checks: XML well-formed · single kinematic root · tree (no cycles, ≤1 parent) ·
expected 19 links / 18 joints · joint link refs resolve · mesh files exist +
per-link AABB (unit sanity) · inertia tensors symmetric, positive-definite, and
obey the triangle inequality (PhysX silently destabilises on invalid inertia).
"""
import os, sys, xml.etree.ElementTree as ET
import numpy as np

HERE = os.path.dirname(os.path.abspath(__file__))
URDF = os.path.join(HERE, "hexabot.urdf")
AABB_MAX = 0.45     # per-LINK mesh; the body hexagon (~0.20 m) is the biggest
...
# --- expected topology for an 18-DOF hexapod ---
check(len(links) == 19, f"19 links", f"expected 19 links, got {len(links)}")
check(len(joints) == 18, f"18 joints (6 legs × 3 DOF)", f"expected 18 joints, got {len(joints)}")
revolute = [j for j in joints if j.get("type") == "revolute"]
check(len(revolute) == 18, "all 18 joints revolute", f"only {len(revolute)} revolute")
...
for l in robot.findall("link"):
    ...
    big = ext.max()
    check(big < AABB_MAX,
          f"{l.get('name'):11s} AABB {ext[0]:.3f}×{ext[1]:.3f}×{ext[2]:.3f} m  ({len(vs)} v)",
...
for l in robot.findall("link"):
    ...
    check(pd and tri and m > 0,
          f"{l.get('name'):11s} m={m:.3f} kg  principal I={w[0]:.2e},{w[1]:.2e},{w[2]:.2e}  (PD+triangle ok)",
