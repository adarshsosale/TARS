#!/usr/bin/env bash
#
# train_and_record_hexabot.sh — one-shot Hexabot (18-DOF hexapod) locomotion pipeline.
#
#   0. Convert hexabot.urdf -> hexabot.usd  (only if the USD is missing)
#   1. Train the PPO policy   (real-time progress streamed to your terminal)
#   2. Export the policy to TorchScript (policy.pt)
#   3. Record a locomotion video for THIS run (unique filename)
#
# Everything for a run (convert/train/export/record logs, the .mp4, and a symlink
# to the raw rsl_rl log dir) lands in  runs/<RUN_ID>/  — kept separate from the
# raw logs/ tree so each run is self-contained.
#
...
#
# Usage:
#   ./train_and_record_hexabot.sh [options]
#
# Options:
#   --name NAME        label appended to the run id (e.g. "tripod-v1")
#   --num-envs N       parallel envs for training        (default 4096)
#   --iters N          training iterations               (default 1000)
#   --cmd-vx V         commanded forward speed for video (default 0.2 m/s)
#   --seconds S        video length in seconds           (default 10)
#   --reconvert        force a fresh URDF -> USD conversion
...
EXPERIMENT="hexabot_flat_direct"
LOGS_BASE="$PROJECT_ROOT/logs/rsl_rl/$EXPERIMENT"
CONDA_ENV="env_isaaclab"
URDF="$PROJECT_ROOT/hexabot_model/isaac/hexabot.urdf"
...
echo "============================================================"
echo "  Hexabot locomotion pipeline"
echo "  run id     : $RUN_ID"
...
[[ -f "$URDF" ]] || { echo "✖ hexabot.urdf not found at $URDF"; exit 1; }
...
# ---------------------------------------------------------------------------
# 0. Convert URDF -> USD (only if missing, or --reconvert).  Joint drive gains
#    here are overwritten by the ArticulationCfg actuators at runtime; we set
#    sane values anyway so the raw USD is usable on its own.
# ---------------------------------------------------------------------------
if [[ $RECONVERT -eq 1 || ! -f "$USD" ]]; then
    run_stage "CONVERT" "$RUN_DIR/convert.log" "Generated USD file:" -- \
        "$ISAACLAB_SH" -p "$ISAACLAB_DIR/scripts/tools/convert_urdf.py" \
        "$URDF" "$USD" --joint-stiffness 12.0 --joint-damping 0.4 --headless \
        || exit 1
    [[ -f "$USD" ]] || { echo "✖ conversion reported success but $USD is missing"; exit 1; }
else
    echo
    echo "  USD already present: $USD  (use --reconvert to rebuild)"
fi
...
if [[ $DO_TRAIN -eq 1 ]]; then
    ...
    run_stage "TRAIN" "$RUN_DIR/train.log" "Training time:" -- \
        "$ISAACLAB_SH" -p "$PROJECT_ROOT/isaac_lab/train_hexabot.py" \
...
run_stage "EXPORT" "$RUN_DIR/export.log" "\[EXPORT\] wrote" -- \
    "$ISAACLAB_SH" -p "$PROJECT_ROOT/isaac_lab/play_hexabot.py" \
...
# ---------------------------------------------------------------------------
VIDEO="$RUN_DIR/hexabot_${RUN_ID}.mp4"
run_stage "RECORD" "$RUN_DIR/record.log" "\[INFO\] wrote" -- \
    "$ISAACLAB_SH" -p "$PROJECT_ROOT/isaac_lab/record_hexabot.py" \
...
echo "------------------------------------------------------------"
echo "  video : $VIDEO"
echo "  logs  : $RUN_DIR/{convert,train,export,record}.log"
